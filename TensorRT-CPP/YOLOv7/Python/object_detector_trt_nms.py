...
class YOLOV7(object):
    def __init__(self, 
            model_weights = '/home/data/hungpham/Count_Box/yolov7/yolov7x-nms-fp16_v2.trt', 
            max_size = 640, 
            names = '/home/data/hungpham/Count_Box/TensorRT-CPP/YOLOv7/Python/data/box.names'):
    ...
    def detect(self, bgr_img, visualize_threshold = 0.5):   
        ...
        for ix in range(num_detection):       # x1, y1, x2, y2 in pixel format
            ...
            img = cv2.rectangle(visualize_img, (int(x1), int(y1)), (int(x2), int(y2)), self.colors[int(cls)], 2)
            img = cv2.putText(img, label, (int(x1), int(y1-10)), cv2.FONT_HERSHEY_SIMPLEX, 1, self.colors[int(cls)], 2, cv2.LINE_AA)
...
if __name__ == '__main__':
    model = YOLOV7()
    img = cv2.imread('/home/data/hungpham/Count_Box/yolov7/vacxin.jpg')
    model.detect(img)
